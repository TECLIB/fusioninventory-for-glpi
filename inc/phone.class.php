...
/**
 * Manage the phone extended information.
 */
class PluginFusioninventoryPhone extends CommonDBTM {
    ...
    * @var string
    */
   static $rightname = 'plugin_fusioninventory_phone';
    ...
    * Display form
    *
    * @param object $item Phone instance
    * @param array $options
    * @return true
    */
   function showForm(Phone $item, $options=[]) {
      Session::checkRight('plugin_fusioninventory_phone', READ);

      $id = $item->getID();
      if (!$data = $this->find("`phones_id`='".$id."'", '', 1)) {
         // Add in database if not exist
         $input = [];
         $input['phones_id'] = $id;
         $_SESSION['glpi_plugins_fusinvsnmp_table'] = 'glpi_phones';
   ...
   function displaySerializedInventory($printers_id) {
      ...
      $a_phoneextend = current($this->find("`printers_id`='".$printers_id."'",
                                               "", 1));

      $this->getFromDB($a_phoneextend['id']);
      ...
      echo "<a href='".$CFG_GLPI['root_doc'].
              ...
              "?itemtype=PluginFusioninventoryPhone".
              "&function=sendSerializedInventory&items_id=".$a_phoneextend['id'].
              "&filename=Phone-".$printers_id.".json'".
   ...
   /**
    * Display extended phone information
    *
    * @param object $item
    */
   static function showInfo($item) {
      ...
      PluginFusioninventoryLock::showLockIcon('Phone');

      $pfPhone = new PluginFusioninventoryPhone();
      $a_phoneextend = current($pfPhone->find(
                                              "`phones_id`='".$item->getID()."'",
                                              "", 1));
      if (empty($a_phoneextend)) {
         return;
      }
      ...
      echo '</td>';
      echo '<td>';
      echo Html::convDateTime($a_phoneextend['last_fusioninventory_update']);
      echo '</td>';
      echo '</tr>';
